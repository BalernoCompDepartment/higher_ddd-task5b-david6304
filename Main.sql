...
SELECT cityName, population/1000 AS populationInThousands
FROM City, Country 
WHERE countryName = 'Ethiopia'
...
SELECT countryName, countryCode, totalPopulation/area AS populationDensity
FROM Country
...
SELECT capital, area*0.39 AS areaInSqMiles
FROM Country
WHERE areaInSqMiles > 500000
...
SELECT countryName, capital, ROUND(longitude/15) AS timeDifferenceFromUK
FROM Country, City
WHERE longitude > 0
AND City.countryCode = Country.countryCode
GROUP BY countryName
ORDER BY timeDifferenceFromUK DESC, countryName ASC;
...
SELECT capital, (latitude-51.5)*113 AS DistanceNorthOfLondon
FROM Country, City
WHERE latitude > 51.5
AND longitude < 35
AND longitude > 0
